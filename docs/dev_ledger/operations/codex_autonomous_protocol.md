...
Autonomous mode lets Codex execute approved Roadmap v2.2 scopes with less manual orchestration while preserving validation, traceability, local commits, roadmap state, and Dev Ledger state.
...
3. Add or update focused tests and acceptance tests for roadmap items.
4. Run focused tests, full unittest discovery, and `.\scripts\verify.ps1`.
5. Update the roadmap only after validation passes.
6. Update the Dev Ledger after each completed mini-epic.
7. Create one local commit per completed green mini-epic.
8. Proceed to the next approved item unless a stop condition is met.
...
- focused tests for the new scope
- acceptance tests for roadmap item behavior and no premature wiring
