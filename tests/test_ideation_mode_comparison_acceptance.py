...
class TestIdeationModeComparisonAcceptance(unittest.TestCase):
    ...
    def test_active_roadmap_is_advanced_to_6_1(self) -> None:
        roadmap = (ROOT / "docs/roadmaps/OOS_roadmap_v2_2_8_weeks_checklist.md").read_text(encoding="utf-8")

        self.assertIn("**0.2.2** Current item: **6.1**", roadmap)
        self.assertIn("**0.2.4** Completed from this roadmap: **10 / 16**", roadmap)
        self.assertIn("**0.2.5** Remaining: **6 / 16**", roadmap)
