...
class TestPatternGuidedIdeationAcceptance(unittest.TestCase):
    ...
    def test_active_roadmap_has_completed_5_1_and_not_regressed(self) -> None:
        ...
        self.assertRegex(source, re.compile(r"\*\*0\.2\.2\*\* Current item: \*\*(5\.2|6\.1|6\.2|7\.1|7\.2|8\.1|8\.2|Completed / final milestone state)\*\*"))
        self.assertRegex(source, re.compile(r"\*\*0\.2\.4\*\* Completed from this roadmap: \*\*(9|10|11|12|13|14|15|16) / 16\*\*"))
        self.assertRegex(source, re.compile(r"\*\*0\.2\.5\*\* Remaining: \*\*(7|6|5|4|3|2|1|0) / 16\*\*"))
        self.assertRegex(
            source,
            re.compile(
