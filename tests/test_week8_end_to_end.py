...
class TestWeek8EndToEnd(unittest.TestCase):
    ...
    def test_end_to_end_dry_run_artifacts_and_weekly_review(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("## Decision Writing Template", founder_checklist)
            self.assertIn("Good `reason`", founder_checklist)
            self.assertIn("Good `next_action`", founder_checklist)
            ...
            self.assertIn(f"Commands below are generated for `{sys.platform}`", founder_checklist)
