...
# Fields that must all be empty for a founder_inbox_v2_index item to be
# considered a synthetic inbox item with no evidence lineage.
_SYNTHETIC_INBOX_LINEAGE_FIELDS: tuple[str, ...] = (
...
class SourceURLTraceabilityArtifactStatus:
    ...
    items_exempt_synthetic_inbox: int = 0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "items_exempt_synthetic_inbox": self.items_exempt_synthetic_inbox,
...
class SourceURLTraceabilityReport:
    ...
    exempt_synthetic_inbox_count: int = 0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "exempt_synthetic_inbox_count": self.exempt_synthetic_inbox_count,
...
def _is_synthetic_inbox_item_without_lineage(
    artifact_key: str, item: dict[str, Any]
) -> bool:
    """Return True only if this is a founder_inbox_v2_index review item with
    no linked IDs and no linked source URLs — i.e., a synthetic inbox item
    with no evidence lineage.

    This is a narrow scanner exemption for items like
    ``decision_recording_commands`` that are generated by the inbox builder
    without upstream evidence references. It does NOT weaken placeholder URN
    detection: items with non-empty ``linked_source_urls`` are still scanned.
    """
    if artifact_key != "founder_inbox_v2_index":
        return False
    for field in _SYNTHETIC_INBOX_LINEAGE_FIELDS:
...
def _check_artifact_source_urls(
    ...
    for item in items:
        ...
        is_synthetic_exempt = _is_synthetic_inbox_item_without_lineage(
            artifact_key, item
        )
        ...
        if is_synthetic_exempt:
            items_exempt_synthetic += 1
            # Synthetic inbox items with no evidence lineage are exempt
            # from missing-source-url checks. Placeholder URN detection
            # remains active if linked_source_urls is non-empty.
            continue
    ...
    return SourceURLTraceabilityArtifactStatus(
        ...
        items_exempt_synthetic_inbox=items_exempt_synthetic,
        issues=issues,
    )
...
def check_source_url_traceability(run_dir: str | Path) -> SourceURLTraceabilityReport:
    ...
    Synthetic founder_inbox_v2_index items with no linked IDs and no linked source
    URLs are exempt from missing-source-url checks. Placeholder URN detection
    remains active for any item with non-empty linked_source_urls.
    ...
    exempt_synthetic_count = sum(
        s.items_exempt_synthetic_inbox for s in artifact_statuses
    )
    ...
    return SourceURLTraceabilityReport(
        ...
        exempt_synthetic_inbox_count=exempt_synthetic_count,
